type ('s, 'a, 'b) muf_node = { init : 's; step : 's * 'a -> 'b * 's }

type ('s, 'a, 'b) instance = { state : 's; node : ('s, 'a, 'b) muf_node }

let init node =
  let node = node () in
  { state = node.init; node }

let step instance args =
  let o, state = instance.node.step (instance.state, args) in
  (o, { instance with state })

let reset instance = ({ instance with state = instance.node.init }, ())
...
let prob_op' n prob args = n (prob, args)
